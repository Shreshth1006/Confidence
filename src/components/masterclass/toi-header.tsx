...
export function TOIHeader() {
  ...
  useEffect(() => {
    // The full header (utility bar + masthead) is ~100px tall.
    // Once user scrolls past that, only the nav strip sticks.
  ...
  return (
    <>
      ...
      <header style={{ width: "100%", background: "#fff", borderBottom: "1px solid #e5e5e5" }}>
        ...
        {/* Row 2: Masthead */}
        <div style={{
          maxWidth: 1400, margin: "0 auto", padding: "10px 16px 9px",
          display: "flex", alignItems: "center", justifyContent: "center", position: "relative",
        }}>
          {/* Left: initiative badge */}
          <div style={{
          ...
          {/* Right: microsite name */}
          <div style={{
            position: "absolute", right: 16, top: "50%", transform: "translateY(-50%)",
            display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1,
          }}>
            <span style={{ fontSize: 8, fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: "#999", lineHeight: 1.4 }}>
              Kids Confidence
            </span>
            <span style={{ fontSize: 8, fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: "#999", lineHeight: 1.4 }}>
              Masterclass
        ...
        {/* Row 3: Nav strip — inline (not sticky) version, hidden when scrolled */}
        {!scrolled && (
          <NavStrip />
        )}
      </header>

      {/* ── Sticky nav strip — appears only after scrolling past the masthead ── */}
      {scrolled && (
        <div style={{
          position: "fixed",
          top: 0,
      ...
      {/* Spacer so content doesn't jump when sticky nav appears */}
      {scrolled && <div style={{ height: 42 }} />}
...
function NavStrip({ compact = false }: { compact?: boolean }) {
  return (
    <nav style={{
      ...
      <div style={{
        maxWidth: 1400, margin: "0 auto", padding: compact ? "0 12px" : "0 12px",
        display: "flex", alignItems: "stretch", whiteSpace: "nowrap",
      }}>
        {/* TOI+ */}
        <a href="#" style={{
          display: "inline-flex", alignItems: "center",
          padding: compact ? "8px 12px 8px 4px" : "9px 14px 9px 4px",
          fontSize: 13, fontWeight: 800, color: "#111", textDecoration: "none",
          borderRight: "1px solid #e5e5e5", marginRight: 4, flexShrink: 0,
        }}>
        ...
        {navItems.map((item) => (
          <a
            ...
            style={{
              ...
              fontSize: 13, fontWeight: 500, color: "#333", textDecoration: "none",
